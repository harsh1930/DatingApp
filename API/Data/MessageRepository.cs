...
namespace API.Data
{
    public class MessageRepository: IMessageRepository
    {
        ...
        {
           return await _context.Connections.FindAsync(conectionId);
        }
        ...
        public async Task<PagedList<MessageDto>> GetMessagesForUser(MessageParams message)
        {
           var query =  _context.Messages.OrderByDescending(x => x.MessageSent).AsQueryable();

            query = message.Container switch
            {
                "Inbox" => query.Where(u => u.RecipientUsername == message.Username && u.RecipientDeleted == false),
                "Outbox" => query.Where(u => u.SenderUsername == message.Username && u.SenderDeleted == false),
                _ => query.Where(u => u.RecipientUsername == message.Username && u.DateRead == null && u.RecipientDeleted == false)
            };

            var messages = query.ProjectTo<MessageDto>(_mapper.ConfigurationProvider);

            return await PagedList<MessageDto>.CreateAsync(messages, message.PageNumber, message.PageSize);
        }
